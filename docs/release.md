...
checks the exact asset set and SHA-256 hashes, and only then creates a draft.

The draft also contains `SHA256SUMS.txt` and a CycloneDX SBOM generated by
Syft 1.52.0 from the frozen pnpm and locked Cargo dependency graphs. GitHub
artifact attestations bind each exact asset to the tagged source commit and
attach the SBOM to the packages. The final workflow job downloads the draft
assets and verifies their hashes, manifest, and provenance again.
...
Check `SHA256SUMS.txt` for the downloaded bytes as well. The draft remains
unpublished until the maintainer reviews its complete assets and notes.
